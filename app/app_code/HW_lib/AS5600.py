...
class RegDescriptor:
    ...
    def get_register(self,obj):
        ...
        buff = obj.i2c.read_i2c_block_data(obj.device,self.reg,self.buffsize)
        v=0
        for b in buff:
            v = v<<8 | b
    ...
    def __set__(self,obj,value):
        ...
        obj.i2c.write_i2c_block_data(obj.device,self.reg,buff) # write result back to the AS5600
...
class AS5600:
    ...
    def get_angle(self):
        # if self.MD == 1:
        pi=np.pi
        angle = float(np.interp(self.ANGLE,[0, 4095],[0.0, 2.0*pi]))
        if self.last_angle < 0.5*pi and angle > 1.5*pi:
            self.round_count-=1
        elif self.last_angle > 1.5*pi and angle < 0.5*pi:
            self.round_count+=1
    ...
    def get_position(self):
        ...
        if np.isnan(angle):
            ...
            pos = (2.0*np.pi*self.round_count)+angle
            return pos
